...
<div class="page-header">
    当前位置：<span class="text-primary">用券送券</span>
</div>
    <div class="page-content">
        <div class="page-toolbar">
            <?php if(cv('sale.coupon')) { ?>
            <?php  if($data['isopenusesendtask']==1) { ?>
        ...
        <form action="" method="post">
            ...
            <table class="table table-hover table-responsive">
                ...
                <tbody>
                <?php  if(is_array($usesendtasks)) { foreach($usesendtasks as $item) { ?>
                <tr>
                    <td>
                        <?php  echo $item['usecouponname'];?>
                    </td>
                    <td>
                    ...
                    <td  style="overflow:visible;position:relative;">
                        <?php if(cv('sale.coupon')) { ?>
                        <a  class='btn btn-default btn-sm btn-operation btn-op' href="<?php  echo webUrl('sale.coupon.usesendtask/edit', array('id' => $item['id'],'page'=>$page))?>" title="<?php if(cv('sale.coupon')) { ?>编辑<?php  } else { ?>查看<?php  } ?>">
                        ...
                        <?php  } ?>
                        <?php if(cv('sale.coupon')) { ?>
                        <a  class='btn btn-default btn-sm btn-operation btn-op' data-toggle='ajaxRemove' href="<?php  echo webUrl('sale.coupon.usesendtask/delete', array('id' => $item['id']))?>" data-confirm='确认要删除吗?？'>
