     ...
     <div class="page-sub-toolbar">
         <span class=''>
            <?php if(cv('sns.adv.add')) { ?>
                <a class="btn btn-primary btn-sm" href="<?php  echo webUrl('sns/adv/add')?>">添加新幻灯片</a>
            <?php  } ?>
        </span>
     </div>
    <form <?php if( ce('sns.adv' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php  echo $item['id'];?>" />
 
                <div class="form-group">
                    <label class="col-lg control-label">排序</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('sns.adv' ,$item) ) { ?>
            ...
            <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        ...
                        <?php  } ?>
                       <input type="button" name="back" onclick='history.back()' <?php if(cv('sns.adv')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default" />
                    </div>
            </div>
