...
<?php  } else if($item['statusvalue'] == 2) { ?>
    ...
    <!--快递 取消发货-->
        ...
        <?php  if($merch_user['finishchecked'] == 1) { ?>
            <?php if(mcv('order.op.finish')) { ?><?php  if(strexists($_W['action'],'order.list')) { ?><?php  } ?>
                <?php  if(!$is_singlerefund) { ?>
                    <a class="btn btn-primary btn-xs" data-toggle='ajaxPost'  href="<?php  echo merchUrl('order/op/finish', array('id' => $item['id']))?>" data-confirm="确认订单收货吗？">确认收货</a>
                <?php  } else { ?>
                    <a class="btn btn-primary btn-xs" data-toggle='ajaxPost'  href="<?php  echo merchUrl('order/op/finish', array('id' => $item['id']))?>">确认收货</a>
                <?php  } ?>
            <?php  } ?>
        <?php  } ?>

        <?php if(mcv('order.op.sendcancel')) { ?><?php  if(strexists($_W['action'],'order.list')) { ?><br/><?php  } ?>
            <a class="btn-xs" style="color: #333;" data-toggle='ajaxModal'  href="<?php  echo merchUrl('order/op/sendcancel', array('id' => $item['id']))?>" >取消发货</a>
        <?php  } ?>
