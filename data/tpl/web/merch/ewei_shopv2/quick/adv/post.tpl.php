...
<div class="page-header">
    当前位置：<span class="text-primary"><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>幻灯片 <small><?php  if(!empty($item['id'])) { ?>(ID: <?php  echo $item['id'];?> 标题: <?php  echo $item['advname'];?>)<?php  } ?></small></span>
</div>
<div class="page-content">
    <div class="page-sub-toolbar">
        <span class=''>
            <?php if(mcv('quick.adv.add')) { ?>
                <a class="btn btn-primary btn-sm" href="<?php  echo webUrl('quick/adv/add')?>">添加新幻灯片</a>
            <?php  } ?>
        </span>
    </div>
    <form <?php if( mce('quick.adv' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php  echo $item['id'];?>" />
 
                <div class="form-group">
                    <label class="col-lg control-label">排序</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( mce('quick.adv' ,$item) ) { ?>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                         <?php if( mce('quick.adv' ,$item) ) { ?>
                        <?php  if($_W['merchid']>0) { ?>
                        <?php  echo tpl_form_field_image2('thumb', $item['thumb'],'',array('dest_dir'=>'merch/'.$_W['merchid']));?>
                        <?php  } else { ?>
            ...
            <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        ...
                        <?php  } ?>
                       <input type="button" name="back" onclick='history.back()' <?php if(mcv('quick.adv')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default" />
                    </div>
            </div>
