...
<div class='fui-page <?php  if(empty($order['single_refundstate'])) { ?>fui-page-current<?php  } ?>' id='page-refund-edit' >
        ...
        <div class="fui-cell">
            ...
            <div class="fui-cell-info">
                ...
                <select id="rtype">
                    ...
                    <?php  } else { ?>
                        <?php  if($order['status']==2 || $order['status']==3 || $order['sendtime']>0) { ?>
...
<div class='fui-page  <?php  if(!empty($order['single_refundstate'])) { ?>fui-page-current<?php  } ?>' id='page-refund-info'>
<div class="fui-header">
    <div class="fui-header-left">
        <a class="back" onclick='history.back()'></a>
    </div>
    <div class="title"><?php  if($order['status']==1) { ?>退款<?php  } else { ?>售后<?php  } ?><?php  if($refund['status']==1) { ?>详情<?php  } else { ?>申请<?php  } ?></div>
    <div class="fui-header-right">&nbsp;</div>
</div>
<div class='fui-content  navbar'>
    <div class='fui-cell-group'>
            <div class='fui-cell noborder' style="padding-bottom: 0">
                <!--<i class='icon icon-info'></i>-->
					<span class="text"><?php  if($refund['status']==0) { ?>等待商家处理<?php  if($order['status']==1) { ?>退款<?php  } else { ?>售后<?php  } ?>申请<?php  } ?>
                        ...
                        <?php  if($refund['status']==-1) { ?>商家已经拒绝<?php  if($order['status']==1) { ?>退款<?php  } else { ?>售后<?php  } ?>申请<?php  } ?>
                        <?php  if($refund['status']==1) { ?>已维权<?php  } ?>
					</span>
            </div>
            <div class='fui-cell'  style="color: #999;font-size: .65rem;line-height: 1.2rem">

                    <?php  if($refund['status']==-1) { ?>
                        拒绝详情：<?php  echo $refund['reply'];?>
                    <?php  } else if($refund['rtype']==0) { ?>
...
<div class='fui-page' id='page-refund-express'>
    ...
    <script language='javascript'>
        require(['biz/order/single_refund'], function (modal) {
            modal.init({orderid: "<?php  echo $order['id'];?>",order_goodsid: "<?php  echo $order_goodsid;?>",single_refundid:"<?php  echo $single_refundid;?>"});
        });
