...
class Index_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
        ...
        if (!(empty($_GPC['cate']))) 
		{
			...
			$condition .= ' AND FIND_IN_SET(' . $_GPC['cate'] . ',user_id)<>0 ';
		}
		...
		{
         $types = pdo_fetch('select id,merchname from ' . tablename('ewei_shop_merch_user') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $row['user_id'], ':uniacid' => $_W['uniacid']));

        $merch_id = pdo_fetch("SELECT * FROM " . tablename("ewei_smerchants_category") . " WHERE merch_id = '" . $row['user_id'] . "' limit 1");
         
         $list[$index]['categoryid'] = $merch_id['name'];
         $list[$index]['merchnames'] = $types['merchname'];
        }
    ...
    $item = pdo_fetch("select * from " . tablename("ewei_shop_merch_user") . " where id=:id and uniacid=:uniacid limit 1", array( ":id" => $id, ":uniacid" => $_W["uniacid"] ));
        ...
        if( $_W["ispost"] ) 
        {
           ...
           // "cateid" => intval($_GPC["cateid"]), 
            ...
            show_json(1, array( "url" => webUrl("smerchants/index", array( "status" => $item["status"] )) ));
        }
        $store = pdo_fetch('SELECT id,storename FROM ' . tablename('ewei_shop_store') . ' WHERE id =:id and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':id' => $item['store_id']));

        $merchres = pdo_fetch('select id,merchname from ' . tablename('ewei_shop_merch_user') . ' where id=:id and uniacid=:uniacid and type=0 limit 1', array(':id' => $item['user_id'], ':uniacid' => $_W['uniacid']));
        $plugins_data = p('Smerchants')->getPluginList();
        $plugins_list = $plugins_data["plugins_list"];
        //$groups = p('Smerchants')->getGroups(); 
        //$category = p('Smerchants')->getCategory();
	    include($this->template());
    }
