...
function merchUrl($do = '', $query = NULL, $full = false)
{
	...
	return str_replace('./index.php', './fund.php', wurl('site/entry', $query));
}

function mce($permtype = '', $item = NULL)
{
	$perm = plugin_run('smerchants::check_edit', $permtype, $item);
	return $perm;
}
...
{
	$perm = plugin_run('smerchants::check_perm', $permtypes);
	return $perm;
}

function mplog($type = '', $op = '')
{
	plugin_run('smerchants::log', $type, $op);
}
...
if ($_W['routes'] != 'smerchants.manage.login') {
	$session_key = '__smerchants_' . $uniacid . '_session';
	...
	if (is_array($session)) {
		...
		if (!is_array($account) || $session['hash'] != md5($account['pwd'] . $account['salt'])) {
			...
			header('location: ' . merchurl('smerchants/manage/login'));
			exit();
		}
