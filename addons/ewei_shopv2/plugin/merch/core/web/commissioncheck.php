...
class Commissioncheck_EweiShopV2Page extends PluginWebPage
{
	protected function main($status = 1) 
	{
		...
		$sql = "select b.*,u.merchname,u.realname,u.mobile from " . tablename("ewei_shop_merch_commission_bill") . " b " . " left join " . tablename("ewei_shop_merch_user") . " u on b.merchid = u.id" . " where 1 " . $condition . " ORDER BY b.id desc ";
		...
		$total = pdo_fetchcolumn("select COUNT(1) from " . tablename("ewei_shop_merch_commission_bill") . " b " . " left join " . tablename("ewei_shop_merch_user") . " u on b.merchid = u.id" . " where 1 " . $condition, $params);
		$pager = pagination2($total, $pindex, $psize);
		$groups = $this->model->getGroups();
		include($this->template("merch/commissioncheck/index"));
	}
	...
	public function detail() 
	{
		...
		$item = $this->model->getOneApplyCommission($id);
        ...
        $list = pdo_fetchall('SELECT c.*,o.finishtime FROM '.tablename('ewei_shop_merch_commission').' c LEFT JOIN '.tablename('ewei_shop_order').' o ON c.ordersn=o.ordersn WHERE o.status = 3 AND c.uniacid = :uniacid AND c.applyid = :applyid LIMIT ' . ($pindex - 1) * $psize.','.$psize,array(':uniacid'=>$_W['uniacid'],':applyid'=>$id));
		if( $_GPC["export"] == "1" ) 
		{
			foreach( $list as &$row ) 
			{
	...
	public function merchpay() 
	{
		...
		pdo_update("ewei_shop_merch_commission_bill", $change_data, array( "id" => $id ));
		$orderids = iunserializer($item["passorderids"]);
        pdo_update("ewei_shop_merch_commission", array( "status" => 3 ), array( "applyid" => $id ));
        show_json(1);
	}
	public function confirm() 
	{
		...
		$item = $this->model->getOneApplyCommission($id);
        $select_order=pdo_fetchcolumn('SELECT COUNT(id) FROM '.tablename('ewei_shop_merch_commission').' WHERE status = 1 AND applyid = :applyid',array(':applyid'=>$id));
		...
		if( $type == 1 ) 
		{
			...
			$pass_data = pdo_fetchcolumn('SELECT SUM(money) FROM '.tablename('ewei_shop_merch_commission').' WHERE applyid = :applyid AND uniacid = :uniacid AND status = 2',array(':applyid'=>$id,':uniacid'=>$_W['uniacid']));
            if(empty($pass_data) || $pass_data < 0)
            {
                show_json(0, "参数错误!");
            }
			$change_data["passrealprice"] = $pass_data;
			$change_data["passrealpricerate"] = $pass_data;
			$change_data["passorderprice"] = 0;
			$change_data["passordernum"] = $item['ordernum'];
			pdo_update("ewei_shop_merch_commission_bill", $change_data, array( "id" => $id ));
//            pdo_update("ewei_shop_merch_commission", array( "status" => 2 ), array( "applyid" => $id ));
        }
		else 
		{
			if( $type == -1 ) 
			{
                ...
                pdo_update("ewei_shop_merch_commission", array( "status" => -1 ), array( "applyid" => $id ));
            }
		}
		show_json(1);
	}
